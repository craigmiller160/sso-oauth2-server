...
class OAuth2ServiceTest {
  ...
  @Mock private lateinit var jwtHandler: JwtHandler
  ...
  fun test_clientCredentials() {
    ...
    `when`(jwtHandler.createAccessToken(clientUserDetails)).thenReturn(Pair(accessToken, tokenId))
    `when`(
  ...
  fun test_password() {
    ...
    val tokenRequest =
      TestData.createTokenRequest(GrantType.PASSWORD, username = user.email, password = "password")
  ...
  fun test_password_wrongPassword() {
    ...
    val tokenRequest =
      TestData.createTokenRequest(GrantType.PASSWORD, username = user.email, password = "password2")
    val ex = assertThrows<InvalidLoginException> { oAuth2Service.password(tokenRequest) }
  ...
  fun test_refresh() {
    setupSecurityContext()
    `when`(jwtHandler.parseRefreshToken(refreshToken, client.id)).thenReturn(tokenData)

    val refreshTokenEntity =
      RefreshToken(
        tokenData.tokenId, refreshToken, client.id, user.id, ZonedDateTime.now(ZoneId.of("UTC")))
