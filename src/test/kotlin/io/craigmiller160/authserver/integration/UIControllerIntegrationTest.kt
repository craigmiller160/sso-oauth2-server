...
class UIControllerIntegrationTest : AbstractControllerIntegrationTest() {
    ...
    fun setup() {
        ...
        client2 = TestData.createClient().copy(
                clientKey = "key2",
                clientRedirectUris = listOf(ClientRedirectUri(0, 0, ""))
        )
    ...
    fun test_getPage_login() {
        val result = apiProcessor.call {
            request {
                path = "/ui/login?client_id=${client1.clientKey}&redirect_uri=${client1.getRedirectUris()[0]}&response_type=code"
            }
            response {
                headers = mapOf(
    ...
    fun test_getPage_noAuthCode() {
        apiProcessor.call {
            request {
                path = "/ui/login?client_id=${client2.clientKey}&redirect_uri=${client2.getRedirectUris()[0]}&response_type=code"
            }
            response {
                status = 401
            }
    ...
    fun test_getPage_other() {
        apiProcessor.call {
            request {
                path = "/ui/foo?client_id=${client1.clientKey}&redirect_uri=${client1.getRedirectUris()[0]}&response_type=code"
            }
            response {
                status = 404
            }
    ...
    fun test_getPage_badParams() {
        apiProcessor.call {
            request {
                path = "/ui/login?client_id=${client1.clientKey}&redirect_uri=${client1.getRedirectUris()[0]}&response_type=code2"
            }
            response {
                status = 401
            }
